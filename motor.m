...
classdef motor < handle 
    ...
    properties (Constant, Hidden)
       ...
       GENERICPIEZOCLASSNAME=   'Thorlabs.MotionControl.GenericPiezoCLI.GenericPiezoCLI';
       

       % Default intitial parameters 
       DEFAULTVEL=60;           % Default velocity
       DEFAULTACC=60;           % Default acceleration
    ...
    methods
        ...
        function updatestatus(h) % Update recorded device parameters in matlab by reading them from the devuce
            ...
            h.triggersettings = h.deviceNET.GetTriggerConfigParams();
        end
        ...
        function tpars =  settriggerconf(h)
            tpars=h.deviceNET.GetTriggerConfigParams()
            %motordirection=Thorlabs.MotionControl.GenericMotorCLI.MotorDirection.Backward
            %methodsview('Thorlabs.MotionControl.GenericMotorCLI.Settings.KCubeTriggerConfigSettings');
            %methodsview('Thorlabs.MotionControl.GenericMotorCLI.Settings');
            %Thorlabs.MotionControl.GenericMotorCLI.Settings.KCubeTriggerConfigSettings();
            %configNET=Thorlabs.MotionControl.GenericMotorCLI.KCubeMotor();
            %absIN=Thorlabs.MotionControl.GenericMotorCLI.Settings.KCubeTriggerConfigSettings.TriggerMode1Str('TrigIN_AbsoluteMove');
            %tpars.Trigger1Polarity = Thorlabs.MotionControl.GenericMotorCLI.Settings.KCubeTriggerConfigSettings.TriggerPolarity.TriggerHigh;
            %tpars.Trigger1Mode=Thorlabs.MotionControl.GenericMotorCLI.Settings.KCubeTriggerConfigSettings.TriggerMode(1);
            %tpars.Trigger2Mode=absIN;
            %h.deviceNET.SetTriggerConfigParams(tpars);
            %updatestatus(h);
        end 
    end
    methods (Static)
        ...
        function loaddlls() % Load DLLs
            if ~exist(motor.DEVICEMANAGERCLASSNAME,'class')
                try   % Load in DLLs if not already loaded
                    ...
                    NET.addAssembly([motor.MOTORPATHDEFAULT,motor.BRUSHLESSMOTORDLL]); 
                    NET.addAssembly([motor.MOTORPATHDEFAULT,motor.GENERICPIEZODLL]); 
